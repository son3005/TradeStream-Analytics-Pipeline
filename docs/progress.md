...
### Phase 1: Kafka Ingestion & Producer Resilience 📨 [CÓ THỂ TỐI ƯU HÓA THÊM 🔄]
...
*   [ ] **Tuning & Management (Chưa làm)**: Cấu hình tối ưu Producer (`acks=all` để bảo toàn dữ liệu, `linger.ms=5` và `batch.size=16384` kết hợp nén dữ liệu `snappy` để giảm tải I/O mạng).
*   [ ] **Tuning & Management (Chưa làm)**: Cấu hình Spark Consumer sử dụng Manual Commit (`enable.auto.commit=false`) thay vì auto commit để đảm bảo xử lý chính xác (At-least-once).
*   [ ] **Resilience & Retry Mechanism**: Bổ sung cơ chế retry với exponential backoff và rate-limiting trong Python Producer để chống lỗi nghẽn hoặc bị chặn IP từ API nguồn.
*   [ ] **Nâng cấp Enterprise (Chưa làm)**: Nghiên cứu & thiết lập **Schema Registry** (Apicurio / Confluent Registry) để quản lý Schema dữ liệu dưới định dạng **Avro/Protobuf** thay vì truyền JSON thô không an toàn.

### Phase 2: Spark Processing & Medallion Architecture ⚡ [CẦN REFACTOR ARCHITECTURE 🔄]
...
*   [ ] **Refactor Logic tính toán (Window Functions)**: Loại bỏ hoàn toàn vòng lặp Python / Python UDFs không tối ưu. Viết lại logic tính toán kỹ thuật (Daily Return, Price Range) sử dụng **Spark SQL Window Functions** kết hợp phân vùng (`partitionBy("symbol").orderBy("fetch_date")`) để đảm bảo không bị tính toán lệch chéo dữ liệu giữa các mã tài sản.
*   [ ] **Thay đổi quy trình Dataflow (Medallion)**: Hủy bỏ việc Spark ghi JDBC trực tiếp xuống PostgreSQL/TimescaleDB ở bước thô. Chuyển sang ghi dữ liệu thô dạng JSON/Parquet lên MinIO (Bronze Layer).
*   [ ] **Xác lập chế độ chạy (Spark Batch Mode)**: Chạy Spark dưới dạng Batch Job được trigger bởi Airflow định kỳ. (Lựa chọn chế độ này để tiết kiệm tài nguyên RAM/CPU thay vì chạy Streaming 24/7).
*   [ ] **Xử lý Schema Drift & DLQ**: Thiết lập cơ chế bắt lỗi khi cấu trúc dữ liệu đầu vào thay đổi đột ngột (Schema Drift) và đẩy các bản ghi lỗi vào Dead Letter Queue (DLQ) trên MinIO.
...
*   [ ] Viết các truy vấn phân tích SQL nâng cao (Window Functions, CTEs, Aggregations) trên Trino.
*   [ ] Thực hiện tối ưu hóa truy vấn bằng cách đọc Execution Plan (`EXPLAIN ANALYZE`).
*   [ ] Tạo các Views phục vụ trực tiếp cho báo cáo và ML.
*   [ ] **Nâng cấp Enterprise (Chưa làm)**: Thực thi **Federated Query** (Truy vấn liên kết) - Thực hiện JOIN trực tiếp giữa bảng dữ liệu lịch sử khổng lồ trong Apache Iceberg (MinIO) và bảng cấu hình nhỏ trong PostgreSQL trên cùng một câu lệnh SQL.
