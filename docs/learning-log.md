...
*Nhật ký học tập chuyên sâu về Kỹ thuật Dữ liệu (Data Engineering) và Hệ thống Phân tán (Distributed Systems) - Lấy cảm hứng từ cuốn sách **Designing Data-Intensive Applications (DDIA)** của Martin Kleppmann.*

---

## 🏗️ PHẦN 1: THIẾT KẾ HỆ THỐNG DỮ LIỆU ĐƠN BÀN (SINGLE-NODE SYSTEMS)

### 1. Phân biệt OLTP vs OLAP dưới góc nhìn Lưu Trữ (DDIA Chương 3)
Việc phân chia hệ thống thành OLTP hay OLAP không chỉ là mục đích sử dụng mà nằm ở **cơ chế đọc/ghi vật lý trên đĩa cứng**:

| Tiêu chí | OLTP (Online Transaction Processing) | OLAP (Online Analytical Processing) |
| :--- | :--- | :--- |
| **Mục đích** | Phục vụ vận hành, giao dịch thời gian thực (đọc/ghi bản ghi ngẫu nhiên). | Phục vụ báo cáo, phân tích và khai phá dữ liệu lớn (quét hàng loạt). |
| **Đặc điểm đọc** | Tìm kiếm một số lượng nhỏ bản ghi bằng khóa (Index Scan), độ trễ thấp. | Quét hàng triệu đến hàng tỷ dòng, nhưng chỉ trên một vài cột cụ thể. |
| **Đặc điểm ghi** | Ghi ngẫu nhiên (Random Write), cập nhật dữ liệu của người dùng trực tiếp. | Ghi hàng loạt (Bulk Load) định kỳ, dữ liệu mang tính bất biến (Immutable). |
| **Layout trên đĩa** | **Row-oriented (Dòng)**: Lưu toàn bộ các cột của dòng cạnh nhau. | **Columnar-oriented (Cột)**: Lưu toàn bộ dữ liệu của cột cạnh nhau. |
| **Đại diện** | PostgreSQL thông thường, MySQL, Oracle. | ClickHouse, Trino, Snowflake, BigQuery, AWS Redshift. |

#### 📊 Biểu diễn vật lý của Row vs Column Layout trên đĩa cứng:
```
ROW-ORIENTED LAYOUT (Ví dụ: PostgreSQL):
[Dòng 1: ID, Symbol, Date, Close, Volume] -> [Dòng 2: ID, Symbol, Date, Close, Volume] -> ...
(Để tính trung bình cột Close, DB buộc phải đọc toàn bộ Dòng 1, Dòng 2 qua mạng I/O -> Rất lãng phí).

COLUMN-ORIENTED LAYOUT (Ví dụ: Parquet / Apache Iceberg):
[Cột ID: 1, 2, 3...] -> [Cột Symbol: AAPL, AAPL, MSFT...] -> [Cột Close: 150.2, 151.1, 405.3...]
(Để tính trung bình cột Close, Query Engine nhảy thẳng đến khối dữ liệu của cột Close để đọc -> Tiết kiệm 90%+ I/O đĩa).
```

#### 🌲 B-Tree vs LSM-Tree: Hai trường phái Storage Engine (DDIA Chương 3)
Khi ghi dữ liệu xuống đĩa cứng, chúng ta phải lựa chọn cách tổ chức chỉ mục (Index) để tối ưu hiệu năng:
*   **B-Tree (Trường phái ghi đè - Ghi chậm, Đọc nhanh)**: Chia đĩa cứng thành các trang (Pages) có kích thước cố định (4KB). Khi ghi dữ liệu mới, DB tìm trang tương ứng để sửa đổi trực tiếp (In-place update).
    *   *Đặc điểm*: Đọc cực kỳ ổn định và nhanh vì cấu trúc cây tự cân bằng và dữ liệu được ghi đè tại chỗ. Tuy nhiên, việc ghi đè ngẫu nhiên có thể dẫn đến phân tách trang (Page Split) làm giảm tốc độ ghi.
    *   *Đại diện*: PostgreSQL (TimescaleDB dưới gầm là Postgres B-Tree), MySQL.
*   **LSM-Tree (Log-Structured Merge-Tree - Ghi siêu tốc, Đọc chậm hơn)**: Dữ liệu mới luôn được ghi append tuần tự vào file log (**WAL**) và bộ nhớ đệm (**MemTable**). Khi MemTable đầy, nó được ghi thành file bất biến dạng sắp xếp (**SSTable**) xuống đĩa. Tiến trình nền chạy **Compaction** để gộp các SSTables nhỏ thành file lớn và loại bỏ dữ liệu cũ.
    *   *Đặc điểm*: Tốc độ ghi nhanh kinh ngạc (vì ghi tuần tự không bao giờ ghi đè). Đọc chậm hơn vì phải tìm kiếm trên nhiều file SSTable cùng lúc.
    *   *Đại diện*: RocksDB, Apache Cassandra.

> [!TIP]
> **Ứng dụng vào TradeStream**: 
> *   Dữ liệu thô từ Kafka bắn ra liên tục sẽ được Spark lưu dạng Parquet/Iceberg trên MinIO theo triết lý **LSM-Tree / Append-Only** để đảm bảo tốc độ ghi tối đa.
> *   Dữ liệu Serving phục vụ trực quan hóa trên Grafana được đẩy về **TimescaleDB** để tận dụng lợi thế đọc nhanh của cơ chế **B-Tree**.
...
*   **ETL (Trường phái truyền thống)**:
    ...
    *   **Điểm yếu**: Mất dữ liệu thô lịch sử, nếu công thức tính toán sai thì không thể chạy lại (backfill) được.
*   **ELT (Trường phái hiện đại - Big Data & Lakehouse)**:
    ...
    *   **Ưu điểm**: Giữ nguyên dữ liệu thô bất biến phục vụ backfill.
...
*   **Nguồn dữ liệu**: Dữ liệu được kéo trực tiếp từ Kafka Topic hoặc gọi API nguồn và lưu thẳng vào Object Storage (MinIO) ở dạng nguyên bản nhất (JSON hoặc CSV thô).
*   **Đặc điểm xử lý**:
    ...
    *   **Partitioning (Phân vùng)**: Thường được chia thư mục theo ngày nạp dữ liệu (`ingest_year=2026/ingest_month=05/ingest_day=18/`).
    *   **Chất lượng**: Dữ liệu cực kỳ "bẩn", chứa bản ghi trùng lặp do mạng chập chờn, dữ liệu null hoặc sai định dạng.
...
*   **Định dạng lưu trữ**: Apache Iceberg để hỗ trợ tính năng ACID, schema evolution và time travel.
*   **Quy trình xử lý chi tiết (Spark Job)**:
    1.  **Schema Enforcement (Áp schema)**: Ép kiểu dữ liệu thô sang đúng kiểu dữ liệu chuẩn (ví dụ: `fetch_date` chuyển từ String sang Date, `close_price` chuyển sang Double/Decimal).
    2.  **Data Cleansing (Làm sạch)**: Loại bỏ các bản ghi bị khuyết thiếu (null) ở các cột quan trọng.
    3.  **Deduplication (Lọc trùng)**: Dựa trên các cột định danh duy nhất (khóa doanh nghiệp) để khử trùng lặp.
    4.  **Data Modeling (Mô hình hóa)**: Spark tách cấu trúc "bảng phẳng" thô ban đầu ra làm các bảng **Fact và Dimension** theo mô hình **Star Schema**.

##### 3. Tầng Gold (Aggregated Layer) - Phục Vụ Kinh Doanh
*   **Vai trò**: Tầng Gold chứa các dữ liệu đã được **tổng hợp sâu (Aggregated)**, sẵn sàng cho người dùng cuối truy vấn vẽ biểu đồ hoặc chạy mô hình AI.
...
*   **Đặc điểm xử lý**: Chạy các câu lệnh SQL tổng hợp phức tạp (như tính toán trung bình giá theo tuần, theo tháng, các chỉ báo kỹ thuật SMA, EMA, RSI). Ghi đè kết quả mới nhất hoặc Upsert nếu có sự thay đổi.
...
*   **Tương lai (Framework-level checks)**: Trong Phase 8, chúng ta sẽ tích hợp **Great Expectations** để định nghĩa các quy tắc kiểm tra tự động trước khi đẩy dữ liệu lên Serving Layer. Nếu dữ liệu vi phạm, Airflow sẽ dừng pipeline và gửi cảnh báo đỏ ngay lập tức.

---

## 🌐 PHẦN 2: THIẾT KẾ HỆ THỐNG PHÂN TÁN (DISTRIBUTED SYSTEMS)

### 1. Phân vùng dữ liệu (Partitioning) & Gom cụm (Bucketing) (DDIA Chương 6)
Để xử lý hàng tỷ dòng dữ liệu, ta bắt buộc phải chia nhỏ tệp dữ liệu trên đĩa cứng:
*   **Partitioning (Phân vùng thư mục vật lý)**: Chia dữ liệu thành các thư mục riêng dựa trên một cột (ví dụ: `/year=2026/month=05/`). Chỉ áp dụng cho các cột có miền giá trị nhỏ (Low Cardinality). Tránh partition theo timestamp vì sẽ tạo ra hàng triệu file nhỏ (Small File Problem).
*   **Bucketing (Clustering - Gom cụm file)**: Băm dữ liệu của một cột có miền giá trị lớn (High Cardinality - ví dụ: băm cột `symbol`) thành một số lượng tệp cố định (ví dụ: 16 files). Spark sẽ gom các bản ghi có cùng mã băm của Symbol vào chung một file để tối ưu hóa phép JOIN và gom nhóm sau này mà không cần Shuffle toàn mạng.

---

### 2. Các cơ chế JOIN phân tán trong Spark (DDIA Chương 6 & 10)
Khi thực hiện JOIN hai bảng nằm rải rác trên nhiều máy chủ, Spark có hai cơ chế thực thi:

#### A. Shuffle Hash Join (Chậm - Tốn tài nguyên mạng)
*   **Cách hoạt động**: Spark băm khóa JOIN của cả hai bảng và truyền dữ liệu của cả hai bảng qua mạng (Shuffle) để gom các bản ghi có chung khóa về cùng một Executor tính toán.
```
Executor 1 (Bảng A)  ───Shuffle qua mạng (Hash Key)───>  Executor 3 (JOIN)
Executor 2 (Bảng B)  ───Shuffle qua mạng (Hash Key)───>  Executor 3 (JOIN)
```

#### B. Broadcast Hash Join (Map-side Join - Siêu nhanh)
*   **Cách hoạt động**: Khi một trong hai bảng rất nhỏ (như bảng `dim_assets` chỉ có vài KB), Spark sẽ copy toàn bộ bảng nhỏ này gửi đến **tất cả các Executors**. Các Executor thực hiện JOIN trực tiếp trên bộ nhớ RAM cục bộ với phần dữ liệu bảng lớn mà không cần truyền bảng lớn qua mạng.
```
               ┌───────────────────────┐
               │ Bảng Nhỏ (dim_assets) │
               └───────────┬───────────┘
                           │ (Broadcast qua mạng 1 lần)
            ┌──────────────┼──────────────┐
            ▼              ▼              ▼
       Executor 1     Executor 2     Executor 3  (JOIN trực tiếp trên RAM cục bộ)
```

> [!IMPORTANT]
> **Tối ưu hóa TradeStream**: Khi thực hiện tính toán ở Silver Layer, ta phải ép cấu hình để Spark chạy **Broadcast Hash Join** giữa `fact_daily_prices` và `dim_assets` bằng cách dùng hàm `broadcast(df_assets)` trong PySpark.

---

### 3. Thiết kế Event Broker: Cơ chế Partition & Offset của Kafka (DDIA Chương 11)
Apache Kafka hoạt động giống như một Log phân vùng được ghi append-only liên tục:
*   **Partition (Làn đường cao tốc)**: Một Topic được chia thành nhiều Partitions. Tin nhắn được phân bổ đều vào các partition này. Nhiều Consumer có thể đọc song song từ các partition khác nhau, giúp scale throughput của hệ thống lên vô hạn.
*   **Offset (Số thứ tự tin nhắn)**: Mỗi tin nhắn đi vào partition được đánh số thứ tự tăng dần vĩnh viễn. Consumer lưu trữ Offset hiện tại của mình. Nếu Consumer bị crash, khi khởi động lại nó chỉ cần yêu cầu Kafka gửi tin nhắn từ `Offset + 1`, giúp đảm bảo tính chịu lỗi (**Fault-tolerance**).
*   **Idempotency (Kháng trùng dữ liệu)**: Trong môi trường mạng không đáng tin cậy, Kafka Producer có thể gửi đúp tin nhắn do không nhận được ACK từ broker. Bằng cách kích hoạt `enable.idempotence=true`, Kafka Broker sẽ tự động phát hiện tin nhắn trùng lặp dựa trên ID của Producer và số Sequence Number để loại bỏ chúng ngay tại cổng vào.

---

### 4. KIẾN THỨC SQL CHUYÊN SÂU DÀNH CHO DATA ENGINEER

SQL là ngôn ngữ giao tiếp bắt buộc của mọi kỹ sư dữ liệu. Dưới đây là các chủ đề nâng cao cần nắm vững:

#### A. Window Functions (Hàm cửa sổ - Trái tim của phân tích chuỗi thời gian)
...
*   **Các hàm kinh điển trong phân tích tài chính**:
    ...
    3.  `AVG(cột) OVER (...)`: Tính trung bình trượt (Moving Average) (ví dụ: SMA 5 ngày gần nhất).

#### B. Stored Procedures & Functions (Thủ tục & Hàm trong DB)
...
*   **Stored Function (Hàm lưu trữ)**: Tương tự như Procedure nhưng bắt buộc phải trả về một giá trị đơn và có thể gọi trực tiếp trong câu lệnh `SELECT` (Ví dụ: `SELECT calculate_return(close, open) FROM daily_prices`).
...
#### C. Tối ưu hóa truy vấn và Đọc Execution Plan (Bản kế hoạch thực thi)
...
*   **Các thuật ngữ cốt lõi cần đọc**:
    1.  **Sequential Scan (Seq Scan - Chậm ❌)**: DB phải duyệt từ đầu đến cuối bảng để tìm dữ liệu. Rất tệ khi bảng có hàng triệu dòng.
...
### 5. KHÁI NIỆM CỐT LÕI VỀ DATA PIPELINE (ĐƯỜNG ỐNG DỮ LIỆU)
...
*   **Batch Processing (Xử lý theo lô)**: Dữ liệu được gom lại thành từng khối lớn và xử lý định kỳ (Ví dụ: chạy job lúc 12h đêm để xử lý dữ liệu của ngày hôm đó). Thích hợp cho báo cáo lịch sử và ML training.
*   **Streaming Processing (Xử lý luồng/Thời gian thực)**: Dữ liệu được xử lý ngay lập tức khi nó vừa phát sinh (Độ trễ tính bằng mili-giây). Thích hợp cho phát hiện gian lận và theo dõi hệ thống.
*   **Idempotency (Tính bất biến/Kháng trùng - Tối quan trọng)**: Một pipeline được gọi là Idempotent nếu bạn chạy từ 1 đến nhiều lần với cùng một tập dữ liệu đầu vào, kết quả đầu ra tại database đích vẫn không đổi và không bị trùng lặp. (Cách chúng ta dùng *Delete-then-Insert* trong Spark chính là để đảm bảo điều này).
*   **Backfill (Bù dữ liệu)**: Quy trình chạy lại pipeline trên một khoảng thời gian lịch sử trong quá khứ khi hệ thống bị mất kết nối hoặc khi thay đổi công thức tính toán.
...
### Ngày 6 (Thiết lập Apache Iceberg Catalog & MinIO)
- **Hoàn thành**: Đã upload tệp `dim_date.csv` tĩnh lên Object Storage MinIO và thiết lập kịch bản kiểm thử kết nối `test_spark_iceberg.py`.
- **Kiến thức về Apache Iceberg Catalog**:
  - Apache Iceberg là định dạng bảng mở (Open Table Format) cho phép quản lý siêu dữ liệu (metadata) của hàng triệu tệp tin phân tán giống như một cơ sở dữ liệu quan hệ SQL thông thường (hỗ trợ ACID transactions, Time Travel, Schema Evolution).
  - Để Spark biết được các bảng Iceberg đang nằm ở đâu và cấu trúc ra sao, Iceberg cần một **Catalog**.
  - **JDBC Catalog**: Chúng ta cấu hình Iceberg sử dụng PostgreSQL (TimescaleDB) làm JDBC Catalog. Khi Spark chạy, Iceberg Engine sẽ tự động tạo ra hai bảng `iceberg_tables` và `iceberg_namespaces` trong Postgres để theo dõi siêu dữ liệu của bảng. Dữ liệu thực tế (data files dạng Parquet) sẽ được lưu trữ tách biệt hoàn toàn trên MinIO (Object Storage).
- **Kiến thức về S3A FileIO**:
  - Để Spark có thể ghi dữ liệu xuống MinIO (tương thích chuẩn Amazon S3), ta phải dùng FileIO implementation của Iceberg dựa trên thư viện Hadoop AWS (`org.apache.hadoop:hadoop-aws`).
  - Cấu hình bắt buộc bao gồm endpoint URL (`http://minio:9000`), Access/Secret Key, và bật `path.style.access=true` để tương thích với các server S3 tự host (như MinIO) thay vì dùng subdomain của Amazon S3.

---

### Ngày 7 (Tối ưu hóa Kafka Ingestion & Producer Resilience)
- **Tối ưu hóa tham số Kafka Producer (Enterprise Tuning):**
  - `acks=all`: Yêu cầu tất cả các partition replicas (bản sao phân tán) phải xác nhận nhận tin nhắn thành công trước khi Producer tiếp tục. Tránh mất dữ liệu khi broker chính (Leader) bị sập.
  - `linger.ms=5` và `batch.size=16384`: Thay vì gửi tin nhắn nhỏ lẻ tẻ, Producer sẽ đợi tối đa 5ms hoặc gom đủ 16KB dữ liệu rồi mới gửi đi trong một lô (Batch). Điều này làm tăng thông lượng (Throughput) mạng đáng kể.
  - `compression.type=snappy`: Sử dụng thuật toán nén Snappy cực nhanh của Google để nén JSON payload thô trước khi truyền qua mạng, giúp tiết kiệm băng thông và tăng tốc độ truyền tải dữ liệu lớn.
- **Cơ chế Retry & Exponential Backoff ở lớp Ingestion:**
  - Để tránh bị các API nguồn (như Yahoo Finance) chặn hoặc sập do quá tải (lỗi HTTP 429 Too Many Requests / Network Timeout), chúng ta triển khai cơ chế thử lại (Retry) với thời gian tăng dần theo cấp số nhân (Exponential Backoff - ví dụ: 1s, 2s, 4s...) kết hợp với rate-limiting tối đa 100 API request đồng thời qua `aiohttp.TCPConnector`.
- **Nghiên cứu về Schema Registry & Định dạng Avro/Protobuf:**
  - *Vấn đề với JSON thô:* JSON tốn dung lượng (do lặp lại tên các trường cho mỗi record) và không có cơ chế bắt buộc cấu hình schema tĩnh (Schema Enforcement) từ phía gửi, dễ dẫn đến lỗi vỡ cấu trúc dữ liệu ở downstream (Spark Consumer).
  - *Giải pháp:* Sử dụng **Schema Registry** kết hợp định dạng **Avro/Protobuf**.
    - **Avro:** Dữ liệu được tuần tự hóa thành mã nhị phân siêu gọn nhẹ (không kèm tên trường), cấu trúc dữ liệu được lưu trữ riêng biệt tại Schema Registry.
    - **Schema Registry (như Apicurio hay Confluent Registry):** Quản lý phiên bản của Schema. Khi gửi tin nhắn, Producer gửi Schema ID kèm theo data nhị phân. Consumer sẽ query Schema Registry để lấy cấu trúc về giải mã (Deserialization). Điều này đảm bảo an toàn kiểu dữ liệu (Data Governance) và khả năng tương thích ngược/xuôi (Schema Evolution).

---

### Ngày 8 (Tối ưu hóa Medallion Pipeline, Spark Tuning & Dead Letter Queue)
- **Cấu hình Shuffle Partitions cho hệ thống nhỏ (`spark.sql.shuffle.partitions=4`):**
  - Mặc định, Spark đặt số lượng phân vùng khi thực hiện các phép toán shuffle (như JOIN, Window Function, `groupBy`) là 200. Đối với các tập dữ liệu nhỏ ở môi trường local, việc phân chia 200 task sẽ tạo ra hàng trăm empty tasks và overhead lập lịch vô ích, làm giảm tốc độ thực thi đáng kể.
  - Cấu hình hạ xuống `4` giúp gộp dữ liệu thành các task có kích thước hợp lý, tăng tốc độ xử lý batch lên gấp nhiều lần.
- **Triển khai Dead Letter Queue (DLQ) & Schema Drift Handling:**
  - Trong luồng xử lý thô từ Bronze sang Silver, bất kỳ thay đổi cấu trúc dữ liệu đột ngột (Schema Drift) hay dữ liệu JSON hỏng hóc đều có thể làm hỏng pipeline downstream.
  - Chúng tôi triển khai cơ chế lọc và bắt lỗi bằng cách kiểm tra kết quả parse JSON: các bản ghi có cột dữ liệu parse bị `null` hoặc thiếu các trường định danh quan trọng (`symbol`, `timestamp`) sẽ lập tức được lọc ra và ghi vào thư mục DLQ trên MinIO (`s3a://lakehouse/dlq/malformed_daily_prices`).
  - Các bản ghi hợp lệ tiếp tục được xử lý, đảm bảo hệ thống tự phục hồi và hoạt động liên tục (Fault Tolerance).
- **Xử lý dữ liệu đến trễ & xáo trộn thứ tự (Late & Out-of-order Data):**
  - Sử dụng cơ chế Deduplication ở mức Batch bằng hàm `dropDuplicates(["symbol", "fetch_date"])`.
  - Kết hợp với sức mạnh của lệnh SQL **`MERGE INTO`** trong Apache Iceberg để thực hiện Upsert (Insert or Update). Nhờ đó, nếu dữ liệu của một ngày đến muộn, nó sẽ tự động cập nhật đè lên bản ghi cũ dựa trên khóa chính `(symbol, date_key)` mà không tạo ra các bản ghi trùng lặp trong Fact Table.

---

### Ngày 9 (Thiết lập kiến trúc Real-Time Hot/Cold Path & Airflow TaskFlow API)
- **Kiến trúc Real-Time Hot/Cold Path**:
  - Tách luồng dữ liệu thành 2 nhánh chính:
    - **Hot Path**: Ticks giao dịch thực tế đi trực tiếp vào TimescaleDB/Kafka phục vụ biểu đồ Real-time (độ trễ dưới 1 giây).
    - **Cold Path**: Dữ liệu thô từ Kafka được gom thành Micro-batch (5 phút) và ghi xuống MinIO (Bronze Layer) bằng Spark, sau đó transform sang nến ngày (Silver Layer - Iceberg) và phục vụ phân tích chuyên sâu.
- **WebSocket Multi-Stream Connection và Cơ chế Reconnection:**
  - Chuyển đổi Crypto Producer từ stream đơn lẻ sang Binance Combined Streams (`/stream?streams=btcusdt@trade/ethusdt@trade`) giúp tiêu thụ song song nhiều đồng coin hiệu quả trên một socket duy nhất.
  - Cấu trúc vòng lặp retry vô hạn kèm backoff giúp duy trì kết nối WebSocket 24/7 liên tục mà không bị đứt gãy luồng dữ liệu khi hạ tầng mạng gặp lỗi tạm thời.
- **Giả lập Stock Ticker thời gian thực với Async Polling:**
  - Viết mới `stock_producer.py` sử dụng `aiohttp` để polling Yahoo Finance Chart API song song định kỳ mỗi 10 giây, mang lại nguồn dữ liệu tick giá chứng khoán tốc độ cao thay vì batch hằng ngày như trước.
- **Gom nhóm Ticks thành OHLCV sử dụng Spark Window Functions:**
  - Thay vì xử lý mảng dữ liệu có sẵn từ Yahoo, Spark Job ở Silver Layer được refactor để tính toán OHLCV bằng Window Functions của Spark SQL:
    - `first("price").over(Window.orderBy("trade_time"))` cho giá Mở cửa (Open).
    - `first("price").over(Window.orderBy(desc("trade_time")))` cho giá Đóng cửa (Close).
    - `max("price")` cho giá Cao nhất (High) và `min("price")` cho giá Thấp nhất (Low).
- **Hiện đại hóa Workflow Orchestration với Airflow 3 TaskFlow API:**
  - Chuyển đổi DAG từ phong cách BashOperator truyền thống sang TaskFlow API (`@dag`, `@task`) giúp code ngắn gọn, tự động quản lý dependencies và xcom truyền tải thông tin.
  - Rút ngắn chu kỳ lập lịch từ daily batch sang 5-minute micro-batch (`*/5 * * * *`) để đồng bộ liên tục dữ liệu từ Kafka xuống Lakehouse.

### Ngày 10 (Refactor cấu hình Spark, Bố cục file & Chuẩn hoá Google-Style Docstrings)
- **Hoàn thành**: 
  - Tạo helper module `src/utils/spark_helper.py` gom toàn bộ cấu hình khởi tạo `SparkSession` (MinIO S3A & Iceberg JDBC Catalog) để tránh trùng lặp code.
  - Refactor 6 Spark jobs và 2 Kafka producers để sắp xếp lại bố cục file: Imports ➔ Environment Config ➔ Helper functions ➔ Main entry point ➔ Execution Guard.
  - Bổ sung **Type Hints** đầy đủ và chuẩn hoá **Google-Style Docstrings** (Args, Returns, Raises) cho toàn bộ 9 file Python của dự án.
  - Phân tách ngôn ngữ trong docstrings rõ ràng: Tiếng Việt cho các hàm nghiệp vụ (business logic) và Tiếng Anh cho mã hạ tầng/kỹ thuật (technical/infrastructure code).
- **Kiến thức**:
  - Việc tổ chức bố cục file (file layout) giúp lập trình viên dễ dàng tìm thấy các hàm bổ trợ và điểm thực thi chính một cách nhanh chóng.
  - Phân chia ngôn ngữ docstrings giúp tài liệu hoá code chuyên nghiệp hơn: business logic được mô tả chi tiết bằng tiếng Việt để dễ trao đổi nghiệp vụ, các module kỹ thuật cấp thấp viết bằng tiếng Anh để đồng bộ với các thư viện quốc tế.
  - Google-style docstrings có cấu trúc rõ ràng giúp sinh tài liệu tự động (như Sphinx) và giúp lập trình viên hiểu ngay kiểu dữ liệu đầu vào/kiểu trả về cùng các ngoại lệ có thể phát sinh.
  - Biên dịch kiểm tra lỗi cú pháp bằng `py_compile` đảm bảo các file code an toàn trước khi đẩy lên container.

---

### Ngày 11 (Phân tích, Thiết kế luồng Hot Path & ML Path)
- **Thiết kế luồng Hot Path (Real-time Sliding Window & Bulk Upsert)**:
  - **Thách thức**: Spark Structured Streaming hỗ trợ xử lý luồng dữ liệu liên tục với độ trễ thấp, nhưng JDBC sink mặc định chỉ hỗ trợ chế độ `append`. Đối với bảng time-series có chỉ báo động (như `crypto_indicators` với khóa duy nhất là `(symbol, window_start)`), việc ghi đè trực tiếp sẽ gây ra lỗi vi phạm khóa chính hoặc trùng lặp dữ liệu khi chạy lại stream.
  - **Giải pháp**: Sử dụng `.writeStream.foreachBatch(write_to_timescale)` để can thiệp vào từng micro-batch DataFrame:
    1. Ghi đè DataFrame của lô hiện tại lên một bảng tạm `crypto_indicators_staging` trên Postgres qua JDBC.
    2. Sử dụng kết nối native Java JDBC (`sparkSession._jvm.java.sql.DriverManager.getConnection`) để thực thi câu lệnh SQL native `INSERT INTO ... ON CONFLICT (symbol, window_start) DO UPDATE SET ...` trực tiếp trên database.
    3. Xóa bảng tạm staging sau khi hoàn tất.
    - Cơ chế này giúp đảm bảo tính **Idempotency** (kháng trùng dữ liệu) tuyệt đối cho luồng ghi streaming.
  - **Tính toán Sliding Window**: Sử dụng watermarking (ví dụ: `1 minute`) kết hợp với cửa sổ trượt (window kích thước 1 phút, trượt 10 giây) giúp Spark tính toán các chỉ số động SMA và VWAP trực tiếp trên RAM và giải phóng trạng thái (state store) cũ một cách tối ưu, tránh tràn bộ nhớ (OOM).

- **Thiết kế luồng ML PATH (OLAP Query via Trino, XGBoost & MLOps)**:
  - **Trích xuất đặc trưng qua Trino**: Thay vì kết nối trực tiếp database quan hệ làm giảm hiệu năng ghi/đọc, ML pipeline kết nối qua **Trino Query Engine** để truy vấn trực tiếp dữ liệu lịch sử ở tầng Silver Iceberg trên MinIO dưới định dạng Parquet tối ưu.
  - **Phân tách dữ liệu Time-series & Tránh rò rỉ dữ liệu (Data Leakage)**: Khi huấn luyện các mô hình chuỗi thời gian, tuyệt đối không dùng chia tách ngẫu nhiên (random split) mà phải dùng chia tách theo thời gian (time-series split - ví dụ: train trên dữ liệu quá khứ, test trên dữ liệu tương lai gần) để tránh mô hình "nhìn trước tương lai" dẫn đến độ chính xác ảo.
  - **Quản lý vòng đời mô hình với MLOps (MLflow)**:
    - Sử dụng **MLflow Tracking Server** để log tham số huấn luyện (hyperparameters), các độ đo (accuracy, precision, recall, f1) và lưu trữ artifacts (model file).
    - Đăng ký mô hình trong **MLflow Model Registry** với tag/alias `Production`.
  - **Thiết kế bảng Dự đoán & Trực quan hóa đối chiếu (Forecast vs Actual)**:
